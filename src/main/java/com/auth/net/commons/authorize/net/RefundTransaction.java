...
public class RefundTransaction {
	...
	public static void main(String[] args) {
		...
		txnRequest.setAmount(new BigDecimal(138.00));
		...
		// Refund
		// Create the payment transaction request
		TransactionRequestType refundTxnRequest = new TransactionRequestType();
		refundTxnRequest.setTransactionType(TransactionTypeEnum.REFUND_TRANSACTION.value());
		refundTxnRequest.setRefTransId(CaptureTransactionId);

		// Make the API Request
		CreateTransactionRequest refundApiRequest = new CreateTransactionRequest();
		refundApiRequest.setTransactionRequest(refundTxnRequest);
		CreateTransactionController refundController = new CreateTransactionController(refundApiRequest);
		refundController.execute(); 

		CreateTransactionResponse refundResponse = refundController.getApiResponse();
		if (refundResponse != null) {
			// If API Response is ok, go ahead and check the transaction response
			if (response.getMessages().getResultCode() == MessageTypeEnum.OK) {
				TransactionResponse result = response.getTransactionResponse();
				if (result.getResponseCode().equals("1")) {
					System.out.println("-------- Refund Transaction Request --------");
					System.out.println("Transaction ID  :" +result.getTransId());
					System.out.println("Account No      : "+result.getAccountNumber());
					System.out.println("Account Type    : "+result.getAccountType());
					System.out.println("AuthCode        : "+result.getAuthCode());
